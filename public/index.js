...
santaImage.src = "/player.png";

const speakerImage = new Image();
speakerImage.src = "/speaker.png";

const walkSnow = new Audio("aot.mp3");

// Ensure audio context is unlocked on first user interaction
document.body.addEventListener("click", () => {
  walkSnow.play().catch((error) => {
    console.error("User interaction required to enable audio:", error);
  });
}, { once: true });
...
const options = {
  appid: "d8037c62e65f46ff9164dc11546c6207",
  channel: "game",
  uid,
  token: null,
};
...
window.addEventListener("keydown", (e) => {
  ...
  if (["a", "s", "w", "d"].includes(e.key) && walkSnow.paused) {
    walkSnow.play().catch((error) => {
      console.error("Failed to play walking sound:", error);
    });
  }
